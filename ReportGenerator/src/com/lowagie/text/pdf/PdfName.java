  ...
  implements Comparable
{
  ...
  public PdfName(String paramString)
  {
    this(paramString, true);
  }

  public PdfName(String paramString, boolean paramBoolean)
  {
    super(4);
    int i = paramString.length();
    if ((paramBoolean) && (i > 127))
      throw new IllegalArgumentException("The name '" + paramString + "' is too long (" + i + " characters).");
    this.bytes = encodeName(paramString);
  }

  public PdfName(byte[] paramArrayOfByte)
  {
    super(4, paramArrayOfByte);
  }

  public int compareTo(Object paramObject)
  {
    PdfName localPdfName = (PdfName)paramObject;
    byte[] arrayOfByte1 = this.bytes;
    byte[] arrayOfByte2 = localPdfName.bytes;
  ...
  public boolean equals(Object paramObject)
  {
    if (this == paramObject)
      return true;
    if ((paramObject instanceof PdfName))
  ...
  public int hashCode()
  {
    ...
    if (i == 0)
    {
      ...
      for (int m = 0; m < k; m++)
        i = 31 * i + (this.bytes[(j++)] & 0xFF);
      this.hash = i;
    }
    return i;
  }

  public static byte[] encodeName(String paramString)
  {
    int i = paramString.length();
    ByteBuffer localByteBuffer = new ByteBuffer(i + 20);
    localByteBuffer.append('/');
    char[] arrayOfChar = paramString.toCharArray();
    for (int j = 0; j < i; j++)
    {
      char c = (char)(arrayOfChar[j] & 0xFF);
      switch (c)
      {
      case ' ':
      case '#':
      ...
      case '}':
        localByteBuffer.append('#');
        localByteBuffer.append(Integer.toString(c, 16));
        break;
      default:
        if ((c >= ' ') && (c <= '~'))
        {
          localByteBuffer.append(c);
        }
        else
        {
          localByteBuffer.append('#');
          if (c < '\020')
            localByteBuffer.append('0');
          localByteBuffer.append(Integer.toString(c, 16));
        }
      }
    }
    ...
    try
    {
      ...
      for (int j = 1; j < i; j++)
      {
        ...
        if (c == '#')
        {
          ...
          int m = paramString.charAt(j + 2);
          c = (char)((PRTokeniser.getHex(k) << 4) + PRTokeniser.getHex(m));
          j += 2;
        }
  ...
  static
  {
    Field[] arrayOfField = PdfName.class.getDeclaredFields();
    staticNames = new HashMap(arrayOfField.length);
    try
    {
      for (int i = 0; i < arrayOfField.length; i++)
      {
        Field localField = arrayOfField[i];
        if (((localField.getModifiers() & 0x19) != 25) || (!localField.getType().equals(PdfName.class)))
          continue;
        PdfName localPdfName = (PdfName)localField.get(null);
        staticNames.put(decodeName(localPdfName.toString()), localPdfName);
      }
    }
 ...
 * Qualified Name:     com.lowagie.text.pdf.PdfName
